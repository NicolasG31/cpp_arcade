...
// Last update Tue Apr  4 11:15:56 2017 Guillaume
//
...
void	run_Player(character &player, map *map)
{
...
void	try_Turning(character &ghost, map *map)
{
...
void	run_Ghost(character &ghost, map *map)
{
  if (ghost.hasBonus != 0)
    {
      clock_t	actTime = clock();
  ...
  if (map->hasWon == false && map->hasLost == false)
      ...
      for (std::vector<character>::iterator node = map->player.begin(); node != map->player.end(); ++node)
	{
	  if ((*node).type == PLAYER)
	    ...
	    {
	      if (map->current_score.score > i * 250)
		run_Ghost((*node), map);
	    }
	  i++;
	}
...
void  show_get_map(map *map, arcade::CommandType com)
{
  ...
  int x = 0;
  int y = 0;
  int pos1 = getFantome(map, 1);
  int pos2 = getFantome(map, 2);
  int pos3 = getFantome(map, 3);
  int pos4 = getFantome(map, 4);
  int index = 0;
  ...
  while (x < map->height)
  {
    ...
    { 
      index = x * map->width + y;
      if (index == pos1 || index == pos2 || index == pos3 || index == pos4)
        gmap->tile[index] = arcade::TileType::EVIL_DUDE;
      else if (map->map[x][y] == EMPTY)
        gmap->tile[index] = arcade::TileType::EMPTY;
      else if (map->map[x][y] == BLOCK) 
        gmap->tile[index] = arcade::TileType::BLOCK;
      else if (map->map[x][y] == S_ITEM) 
...
void  show_where_an_i(map *map, arcade::CommandType com)
{
  ...
  arcade::WhereAmI *where = new arcade::WhereAmI[sizeof(arcade::CommandType) + sizeof(uint16_t) + (sizeof(arcade::Position) * 1)];
  int i = 0;
  arcade::Position pos;
  ...
  mread.my_write_mouli(1, where, sizeof(arcade::CommandType) + sizeof(uint16_t) + (sizeof(arcade::Position) * 1));
}
